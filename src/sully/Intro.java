...
public class Intro {
	...
	static boolean runningIntro = true; // [Rafael, the Esper]: Necessary to control the execution flow
	//
	//
	//
	...
	public static void startIntro()
	{
		...
		// Rafael: This is nasty, but the only practical way to interrupt the 
		// intro scene smoothly without changing a lot of code
		thread.stop(); 
		//thread.interrupt();
	...
	public static void doIntroAnimation() throws InterruptedException {
			...
			//entity.get(stan).visible = false; // [Rafael, the Esper]: To avoid strange behavior of Stan
	...
	static void LoadGame()
	{
		...
		_title_menu_load_cancel = false; // [Rafael, the Esper]
		...
		_title_menu = 0;
		if(_title_menu_load_done) { // [Rafael, the Esper] (because map doesn't interrupt execution anymore)
			MenuOn();
			return;
		}
		DoMenu();
	...
	/*void cleanupIntro() [Rafael, the Esper]: Not needed 
	{
		int i;
