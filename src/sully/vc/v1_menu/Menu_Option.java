...
public class Menu_Option {
	...
	public static void MenuControlOption()
	{
		...
		if (menu_sub == 0)
		{
			...
			menu_item = ret[2]; // [Rafael, the Esper]
		...
		else if (menu_sub == 2)
		{
			...
			menu_sub = ret[1]; // [Rafael, the Esper]
			global_noscroll = ret[2]==0?false:true; // [Rafael, the Esper]			
		}
		else if (menu_sub == 3)
		{
			int ret[] = MenuControlTwoArrows(menu_sub, 4, global_menuluc, 11);
			menu_sub = ret[1]; // [Rafael, the Esper]
			global_menuluc = ret[2]; // [Rafael, the Esper]
		}
		else 
		{
		...
		if (MenuConfirm())
		{
			if (menu_sub == 0)
			{
				...
				if (menu_item < 4)
				{
					menu_start = menu_colour[menu_item].getRGB(); // [Rafael, the Esper] Set
	...
	public static void MenuControlOptionVol()
	{
		...
		int ret[] = MenuControlFastArrows(menu_sub, 100, menu_item, 3); // [Rafael, the Esper]
		menu_sub = ret[1]; // [Rafael, the Esper]
		menu_item = ret[2]; // [Rafael, the Esper]
		if (ret[0] ==0)
		{
	...
	public static void MenuControlOptionRGB()
	{
		...
		int ret[] = MenuControlFastArrows(menu_sub, 255, menu_cast, 3); // [Rafael, the Esper]
		menu_sub = ret[1]; // [Rafael, the Esper]
		...
		if (MenuCancel())
		{
			...
			menu_cast = 0-1;
			menu_sub = 0;
			menu_colour[menu_item] = new Color(menu_start); // [Rafael, the Esper] unSet
			menu_start = 0;
			menu_idx = MenuGet("Option");
		}
	}
