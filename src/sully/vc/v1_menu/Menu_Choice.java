...
public class Menu_Choice {
	...
	//log( "MIB 2" );
			if (IsInMenu()) 
				callfunction(master_menus[mini_hold].draw_func); // [Rafael, the Esper]: Added callfunction
	//log( "MIB 3" );
	...
	//log( "MIB 6" );
			// Rafael: TODO Implement this later. // current = GetCurrentToken(wrapinput, "&", cursor_pos);
	//log( "MIB 7" );
	...
	//log( "MIB 9" );
			...
			{
				// Rafael: TODO Implement this later. //printstring(xpos + 12, ypos + 12 + ((i - start) * menu_fonth), screen, menu_font[0], gettoken(wrapinput, "&", i));
				if (i >= start + 4) i = linenum + 1;
			}
	//log( "MIB 10" );
			// Rafael: TODO Implement this later. //BlitCursor(xpos + 12, ypos + 12 + ((current - start) * menu_fonth), menu_font[0], cursor_pos - gettokenPos(wrapinput, "&", current, 0), gettoken(wrapinput, "&", current));
	//log( "MIB 11" );
			showpage();
	//log( "MIB 12" );
	...
	/* TODO Rafael: I'm not in the mood to dealing with all this keyboard stuff right now	
