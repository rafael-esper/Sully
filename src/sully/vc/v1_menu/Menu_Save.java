...
public class Menu_Save {
		...
		try {
			...
			if (MenuConfirm())
			{
				...
				else if (menu_sub!=0) // If loading, and there is 1 or more saved games
				{
					...
					_title_menu_load_done = true; // [Rafael, the Esper]
				}
				else MenuAngryBuzz();
			}
	...
	// Checks for existence of save files. Can be fooled by renaming or deletion.
	{
		...
		try {
			...
			int filecount = count;
			log("MenuSave::Found " + filecount + " files.");
	...
	public static void MenuSaveGame(int slot) throws IOException
	{
		...
		ExtendedDataOutputStream edos = new ExtendedDataOutputStream(fos); // [Rafael, the Esper]
		...
		// Map Data
			if(current_map == null) { // [Rafael, the Esper]: Just for robustness
		...
		// Footer
			//[Rafael, the Esper] edos.writeSignedIntegerLittleEndian(FileCurrentPos(savegame));
		edos.close();
		//FreeImage(mini);
	...
	public static void MenuLoadGame(int slot) throws IOException
	{
		int xc, yc;
		String mapname;
		ExtendedDataInputStream edis = 
				new ExtendedDataInputStream(
						load("save/save"+ThreeDigit(slot)+".sve").openStream()); // [Rafael, the Esper]
		
		// Header Size
		int hsize = edis.readSignedIntegerLittleEndian(); // [Rafael, the Esper]
		//log("Header size: " + hsize);
		for(int i=0;i<hsize-4; i++) {
			edis.readByte();
		}
		
		//[Rafael, the Esper] FileSeekPos(savegame, edis.readSignedIntegerLittleEndian(), SEEK_SET);
		// Party Data
			curpartysize = edis.readSignedIntegerLittleEndian();
			//log("curpartysize: " + curpartysize);
			...
			for (yc = 0; yc <  MAX_CAST; yc++)
			{
				if(master_cast[yc]==null) // [Rafael, the Esper]: Just for robustness
		...
		// Map Data
			...
			edis.readShort(); // [Rafael, the Esper]
		...
		// Footer
//[Rafael, the Esper]			if (FileCurrentPos(savegame) == edis.readSignedIntegerLittleEndian())
			//[Rafael, the Esper]			{
				edis.close();
				...
				//[Rafael, the Esper]			}
				//[Rafael, the Esper]else
				//[Rafael, the Esper]{
				//[Rafael, the Esper]edis.close();
				//[Rafael, the Esper]error("Errors have occurred reading this save format, load aborted");
				//[Rafael, the Esper]}
	...
	public static void MenuLoadSaveDisplay(int slot) throws IOException
	{
		...
		ExtendedDataInputStream edis = 
				new ExtendedDataInputStream(
						load("save/save"+ThreeDigit(slot + 1)+".sve").openStream()); // [Rafael, the Esper]
